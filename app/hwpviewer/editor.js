...
async function initialize() {
    try {
        ...
        // 자동 파일 로드 (empty.hwp)
        await loadRemoteFile('empty.hwp');
    } catch (error) {
...
/**
 * 전역 doc 인스턴스가 생성된 후 UI 및 상태를 초기화한다.
 */
function afterDocumentLoaded(newDoc, fileName, parseTime) {
    if (doc && doc !== newDoc) {
        doc.free();
    }
    doc = newDoc;
    ...
    // 문서 정보 갱신
    displayDocInfo(fileName, parseTime);
    ...
    console.log(`파일 로드 완료: ${fileName} (${doc.pageCount()}페이지, ${parseTime.toFixed(2)}ms)`);
}

/**
 * 서버에서 HWP 파일을 가져와 로드한다.
 */
    ...
    try {
        ...
        if (!response.ok) {
            if (response.status === 404) {
                console.warn(`자동 로드 파일 없음: ${url}`);
                return;
            }
            throw new Error(`파일을 가져오는데 실패했습니다: ${response.statusText}`);
        }
        ...
        const newDoc = new HwpDocument(data);
        newDoc.convertToEditable();
        const parseTime = performance.now() - startTime;

        const fileName = url.split('/').pop();
        afterDocumentLoaded(newDoc, fileName, parseTime);
    } catch (error) {
        console.error('원격 파일 로드 실패:', error);
        // 자동 로드 실패 시 사용자에게 알리지 않고 수동 업로드를 기다림
    } finally {
        showLoading(false);
    }
}
...
async function handleFileUpload(event) {
    ...
    try {
        ...
        afterDocumentLoaded(newDoc, file.name, parseTime);
    } catch (error) {
