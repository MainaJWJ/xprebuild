...
export const XP_SHADERS = {
    ...
    combined: `
      ...
      void main() {
        ...
        mEff *= intensity;
        vec2 sUv = uv;
        if (glitchEnabled && random(vec2(time * 0.1, 0.0)) < syncErrorFreq * (1.0 + mEff) * intensity) sUv.x += (random(vec2(time, sUv.y)) - 0.5) * 0.1;
        if (crtEnabled) {
        ...
        if (glitchEnabled && lineDisplacement > 0.0) {
            if (random(vec2(floor(sUv.y * 50.0), time)) < 0.1 * (1.0 + mEff)) sUv.x += (random(vec2(time, sUv.y)) - 0.5) * lineDisplacement * (1.0 + mEff) * intensity;
        }
        ...
        if (glitchEnabled) {
            float effBitCrush = bitCrushDepth * intensity;
            if (effBitCrush > 0.0) { float lv = max(2.0, 32.0 - effBitCrush); col = floor(col * lv) / lv; }
            if (random(vec2(time, floor(sUv.y * 10.0))) < signalDropoutFreq * intensity) col *= random(sUv + time) > 0.5 ? 0.0 : 1.5;
        }
    ...
    glitch: `
      ...
      void main() {
        float mEff = getInteractionEffect(vUv, mouse, radiusPx, resolution.x/resolution.y, effectScale, interactionShape);
        vec2 sUv = vUv;
        if (random(vec2(time * 0.1, 0.0)) < syncErrorFreq * (1.0 + mEff) * intensity) sUv.x += (random(vec2(time, sUv.y)) - 0.5) * 0.1;
